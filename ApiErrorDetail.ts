...
export const chatApi = {
  sendMessage: (receiverId: string, messageText: string, screeningId?: string, attachmentUrl?: string) =>
    apiFetch<any>('/api/v1/chat/messages', {
      method: 'POST',
      body: JSON.stringify({ receiverId, messageText, screeningId, attachmentUrl }),
    }),
  ...
  getConversations: () =>
    apiFetch<any[]>('/api/v1/chat/conversations', {
      method: 'GET',
    }),

  getScreeningMessages: (screeningId: string) =>
    apiFetch<any[]>(`/api/v1/chat/screening/${screeningId}`, {
      method: 'GET',
    }),
