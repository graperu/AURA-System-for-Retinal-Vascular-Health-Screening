...
export const CDSDashboardPage: React.FC = () => {
  ...
  const handleSaveFeedback = async (feedback: DoctorFeedback) => {
    try {
      ...
      const aiRiskLevel = analysisResult?.cardiovascularRisk.level || 'High';
      await feedbackApi.submit({
        screeningId: feedback.analysisId || '00000000-0000-0000-0000-000000000000',
        aiRiskLevel,
        doctorRiskLevel: feedback.adjustedCardioRisk || aiRiskLevel,
        isAccurate: feedback.decision === 'APPROVED',
        feedbackNotes: feedback.clinicalNotes || 'Bác sĩ đã xác nhận kết quả chẩn đoán',
        vesselAnnotationData:
          feedback.lesionAnnotations && feedback.lesionAnnotations.length > 0
            ? JSON.stringify(feedback.lesionAnnotations)
            : null,
      });
