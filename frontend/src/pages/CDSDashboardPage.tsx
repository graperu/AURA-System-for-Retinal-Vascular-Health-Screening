...
import {
  VascularTrendChart,
  VascularPoint,
} from "../components/VascularTrendChart";
...
export const CDSDashboardPage: React.FC = () => {
  ...
  const [vascularPoints, setVascularPoints] = useState<VascularPoint[]>([]);
  ...
  const [feedbackSuccessMsg, setFeedbackSuccessMsg] = useState<string>(
    "Đã lưu xác nhận chẩn đoán thành công",
  );

  const loadPatientDetails = useCallback(
    async (patientId: string, summaryFallback?: DoctorPatientSummary) => {
      ...
      try {
        ...
        if (profileRes.success && profileRes.data) {
          ...
          const mapped: PatientProfile = {
            ...
            hasHypertension:
              d.hasHypertension ?? summaryFallback?.hasHypertension ?? null,
        ...
        if (
          screeningsRes.success &&
          Array.isArray(screeningsRes.data) &&
          screeningsRes.data.length > 0
        ) {
          const latestScreening = screeningsRes.data[0];
          setAnalysisResult(
            mapScreeningToAIRiskResult(
              latestScreening,
              latestScreening.imageUrl,
            ),
          );
        } else {
          setAnalysisResult(null);
        }
        const tl = await timelineApi.doctorPatient(patientId);
        if (tl.success && Array.isArray(tl.data)) setVascularPoints(tl.data);
        else setVascularPoints([]);
      } catch (err) {
  ...
  const fetchAssignedPatients = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        ...
        setPatientsError(
          res.message || "Không thể tải danh sách bệnh nhân được phân công.",
        );
      }
    } catch (err) {
      setPatientsError(
        err instanceof Error ? err.message : "Lỗi kết nối máy chủ phân công.",
      );
    } finally {
  ...
  const handlePatientSelectChange = (patientId: string) => {
    ...
    const selectedSummary = assignedPatients.find(
  ...
  const handleStartAnalysis = async (request: FundusAnalysisRequest) => {
    ...
    setAnalysisErrorMsg(null);
    setAnalysisProgress({
      status: "Khởi tạo kết nối AI Microservice...",
      percent: 15,
    });

    try {
      setAnalysisProgress({
        status: "Đang gửi ảnh đến AURA AI Core cho bệnh nhân...",
        percent: 45,
      });

      // Call doctor-specific screening endpoint
      const res = await doctorApi.createScreeningForPatient(
        selectedPatientId,
        request.imageUrl,
      );
      ...
      if (enableMockAi) {
        console.warn(
          "VITE_ENABLE_MOCK_AI is enabled. Falling back to local mock AI engine.",
        );
        const result = await MockAIService.runFundusAnalysis(
          request,
          (status, percent) => {
  ...
  const handleSaveFeedback = async (feedback: DoctorFeedback) => {
    try {
      ...
      await feedbackApi.submit({
        screeningId:
          feedback.analysisId || "00000000-0000-0000-0000-000000000000",
      ...
      <div className="flex flex-col items-center justify-center min-h-[400px] bg-white border border-[#CCFBF1] rounded-3xl p-10 shadow-medical-sm text-center space-y-4">
        ...
        <div className="space-y-1">
          <h3 className="text-base font-bold text-slate-800">
            Đang tải danh sách bệnh nhân phân công
          </h3>
          <p className="text-xs text-slate-500">
            Hệ thống đang truy xuất hồ sơ y tế bệnh nhân thuộc quyền quản lý của
            Bác sĩ...
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-[450px] bg-white border border-[#CCFBF1] rounded-3xl p-10 shadow-medical-sm text-center space-y-5">
        ...
        <div className="space-y-2 max-w-lg">
          <h3 className="text-lg font-bold text-slate-900">
            Chưa Có Bệnh Nhân Được Phân Công
          </h3>
          <p className="text-xs text-slate-600 leading-relaxed">
            Tài khoản Bác sĩ của bạn hiện chưa được phân công quản lý bệnh nhân
            nào trong hệ thống AURA. Theo quy định bảo mật RBAC, Bác sĩ chỉ có
            quyền truy cập hồ sơ và ca khám của các bệnh nhân đã được phân công
            tiếp nhận.
          </p>
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl text-[11px] text-slate-500 text-left space-y-1 mt-3">
            <p className="font-semibold text-slate-700">
              📌 Hướng dẫn kích hoạt phân công:
            </p>
            <p>
              1. Liên hệ Quản trị viên (Admin) hoặc Lễ tân phòng khám để tiếp
              nhận danh sách chỉ định.
            </p>
            <p>
              2. Khi bệnh nhân đặt lịch hoặc được phân luồng, danh sách sẽ tự
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-[#CCFBF1] rounded-2xl p-5 shadow-medical-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-base font-bold text-[#134E4A]">
                {activePatient.fullName || "Chưa cập nhật tên"}
              </h2>
              ...
              <span className="text-xs text-slate-500 font-medium">
                ...
                {activePatient.gender === "Female"
                  ...
                  : activePatient.gender === "Male"
                    ? "Nam"
                    : activePatient.gender || "Chưa cập nhật"}
                )
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-0.5">
              ...
              | Tiền sử:{" "}
              {activePatient.hasDiabetes === true
                ? "Đái tháo đường"
                : activePatient.hasDiabetes === false
                  ? "Không ĐTĐ"
                  : "Chưa cập nhật"}
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 font-semibold hidden xl:block">
              Bệnh nhân phân công:
            </span>
            <select
            ...
            >
              {assignedPatients.map((p) => (
                <option key={p.patientId} value={p.patientId}>
                  {p.fullName || "Bệnh nhân"} ({p.mrn || "Chưa có MRN"}) —{" "}
                  {p.age ? `${p.age}t` : "N/A"} ({p.screeningCount} ca khám)
                </option>
              ))}
            </select>
            <button
          ...
          {isScreeningLoading ? (
            <div className="bg-white border border-[#CCFBF1] rounded-2xl p-8 shadow-medical-sm text-center flex flex-col items-center justify-center min-h-[380px] space-y-3">
              ...
              <p className="text-xs text-slate-500 font-medium">
                Đang tải lịch sử ca sàng lọc của bệnh nhân...
              </p>
            </div>
          ) : analysisResult ? (
              ...
              <div className="space-y-1.5 max-w-md">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  ...
                  <strong className="text-slate-700">
                    {activePatient.fullName || activePatient.mrn || "này"}
                  </strong>{" "}
                  chưa có ca sàng lọc nào trong hệ thống. Bác sĩ có thể tải lên
                  ảnh chụp đáy mắt (Fundus) ở bảng bên trái để thực hiện phân
                  tích và đánh giá nguy cơ vi mạch.
                </p>
              </div>
            </div>
          )}
      ...
      <VascularTrendChart
        points={vascularPoints}
        title="Timeline xu hướng vi mạch bệnh nhân (FR-17)"
      />
