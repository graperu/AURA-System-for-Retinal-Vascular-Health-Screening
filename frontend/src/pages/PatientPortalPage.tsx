...
export const PatientPortalPage: React.FC<PatientPortalPageProps> = ({
  ...
  const handleSendChatMessage = async (e: React.FormEvent) => {
    ...
    if (!assignedDoctorId) {
      ...
      setAnalysisErrorMsg(
        "Chưa có bác sĩ được phân công nên không thể gửi tin nhắn.",
      );
      return;
    }
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-900">
                    Quản Lý Gói Dịch Vụ & Lượt Phân Tích (FR-11, FR-12)
                  </h2>
                  <p className="text-xs text-slate-500">
                    Theo dõi số lượt phân tích thị giác AI, gói cước kích hoạt
                    và lịch sử thanh toán qua cổng VNPay / MoMo.
                  </p>
                </div>
              </div>
              <button
                onClick={() => setIsCreditModalOpen(true)}
                className="px-5 py-2.5 bg-cyan-700 hover:bg-cyan-800 text-white font-bold text-xs rounded-xl shadow-md flex items-center gap-2 transition-all"
              >
                <CreditCard className="w-4 h-4" /> Mua Hoặc Gia Hạn Gói
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-5 rounded-2xl bg-gradient-to-br from-cyan-50 to-teal-50 border border-teal-200/80">
                ...
                <p className="mt-1 text-[11px] text-slate-500">
                  Áp dụng cho mọi phân tích ảnh võng mạc OD/OS
                </p>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs">
                ...
                <p className="mt-2 text-[11px] text-slate-500">
                  Tự động kích hoạt ngay sau thanh toán
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                <Zap className="w-4 h-4 text-amber-500" /> Danh Sách Gói Thuê
                Bao Đã Mua
              </h3>
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
                    <tr>
                      ...
                      <th className="p-3.5">Số Lượt Còn Lại</th>
                  ...
                  <tbody className="divide-y divide-slate-100 font-medium">
                    {subscriptions.length === 0 ? (
                      <tr>
                        ...
                        >
                          Chưa đăng ký gói dịch vụ nào. Hãy nhấn Mua Thêm Gói để
                          nạp lượt phân tích.
                        </td>
                      </tr>
                    ) : (
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  <History className="w-4 h-4 text-cyan-700" /> Lịch Sử Thanh
                  Toán & Giao Dịch Hóa Đơn (FR-12)
                </h3>
                <button
