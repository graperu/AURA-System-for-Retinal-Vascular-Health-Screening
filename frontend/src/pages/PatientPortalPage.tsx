...
export const PatientPortalPage: React.FC<PatientPortalPageProps> = ({
  ...
  const [analysisResult, setAnalysisResult] =
    useState<AIRiskResult>(MOCK_SAMPLE_RESULT);
  ...
  useEffect(() => {
    const fetchRealData = async () => {
      ...
      // Fetch profile
      try {
        const creditRes = await billingApi.credits();
        if (creditRes.success && creditRes.data) {
          setUserCredits(creditRes.data.remainingCredits ?? 0);
          setLowCreditWarning(!!creditRes.data.lowCreditWarning);
          setSubscriptions(creditRes.data.subscriptions || []);
        }
        const tl = await timelineApi.mine();
        if (tl.success && Array.isArray(tl.data)) setVascularPoints(tl.data);
      } catch (e) {
        console.warn("Could not fetch billing/timeline:", e);
      }
  ...
  const handleStartAnalysis = async (request: FundusAnalysisRequest) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Backend call failed, using mock AI fallback:",
          backendErr,
        );
      }

      if (!result) {
        result = await MockAIService.runFundusAnalysis(
          request,
          (status, percent) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="bg-amber-50 border-2 border-amber-400 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="text-xs font-bold text-amber-950">
              Cảnh báo số lượt sắp hết (FR-12)
            </h4>
            <p className="text-xs text-amber-800">
              Bạn còn ≤ 1 lượt phân tích. Hãy gia hạn gói để không gián đoạn tầm
              soát.
            </p>
          </div>
          <button
          ...
          {isProfileLoading ? (
            ...
            <div className="bg-white p-8 rounded-2xl border border-red-200 shadow-xs flex flex-col items-center justify-center space-y-3">
              ...
              <p className="text-sm font-bold text-slate-800">
                Không thể tải hồ sơ y tế từ máy chủ.
              </p>
              <button
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                      ...
                      <span className="text-slate-400 font-mono-data">
                        •{" "}
                        {patient.updatedAt
                          ? `Cập nhật lần cuối: ${new Date(patient.updatedAt).toLocaleString("vi-VN")}`
                          : "Chưa có cập nhật"}
                      </span>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                ...
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                  ...
                  <span
                    ...
                    title={patient.address || "Chưa cập nhật"}
                  >
              ...
              <div className="p-5 bg-teal-50/50 rounded-2xl border border-teal-100 space-y-4">
                <h3 className="text-xs font-bold text-teal-900 uppercase tracking-wider flex items-center gap-2">
                  <Activity className="w-4 h-4 text-teal-700" /> Chỉ Số Sinh
                  Hiệu & Lâm Sàng Gần Nhất
                </h3>
                    ...
                    patient.diastolicBp != null ? (
                      ...
                      <>
                        ...
                        <span className="text-[11px] text-slate-400 block mt-0.5">
                          Vui lòng cập nhật khi có kết quả đo
                        </span>
                      </>
                    )}
                  </div>
                  <div className="p-4 bg-white rounded-xl border border-teal-200 shadow-xs">
                    ...
                    {patient.hba1c != null ? (
                      ...
                      <>
                        ...
                        <span className="text-[11px] text-slate-400 block mt-0.5">
                          Chưa có dữ liệu xét nghiệm máu
                        </span>
                      </>
                    )}
                  </div>
              ...
              <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
                  <Heart className="w-4 h-4 text-red-500" /> Tiền Sử Bệnh Lý Mạn
                  Tính
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                  <div className="p-3 bg-white rounded-xl border border-slate-200 flex items-center justify-between">
                    ...
                    {renderConditionStatus(
                      ...
                      patient.diabetesType
                        ? `${patient.diabetesType}${patient.diabetesDurationYears ? ` - ${patient.diabetesDurationYears} năm` : ""}`
                        : undefined,
                    )}
                  </div>
                  <div className="p-3 bg-white rounded-xl border border-slate-200 flex items-center justify-between">
                  ...
                  <div className="p-3 bg-white rounded-xl border border-slate-200 flex items-center justify-between">
                    ...
                    <strong
                      ...
                      title={patient.allergies || "Chưa khai báo"}
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                  ...
                  <p className="text-slate-800">
                    {patient.currentMedications || "Chưa khai báo"}
                  </p>
                </div>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                  <span className="text-slate-500 font-semibold block">
                  ...
                  <p className="text-slate-800">
                    {patient.emergencyContactName
                      ? `${patient.emergencyContactName} (${patient.emergencyContactPhone || "Chưa cập nhật SĐT"})`
                      : "Chưa khai báo"}
                  </p>
                </div>
              </div>
      ...
      {activeView === "scan-history" && (
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-6">
            ...
            <VascularTrendChart points={vascularPoints} />
          </div>
        </div>
      )}
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-900">
                    Quản lý gói cước & lượt khám (FR-11, FR-12)
                  </h2>
                  <p className="text-xs text-slate-500">
                    Số dư:{" "}
                    <strong className="text-teal-600 font-mono-data text-sm">
                      {userCredits} lượt
                    </strong>
                    {lowCreditWarning ? " — cảnh báo ≤ 1 lượt" : ""}
                  </p>
                </div>
              </div>
              <button
              ...
              >
                <CreditCard className="w-4 h-4" /> Mua / gia hạn
              </button>
            </div>
            <p className="text-[11px] text-slate-500">
              Cổng: VNPay, MoMo, ZaloPay, VietQR, Visa/Mastercard. Voucher:
              AURA10 / TET2026 / FAMILY15. Có xử lý treo, thất bại và hoàn tiền.
            </p>
            <div className="space-y-2">
              ...
              {subscriptions.map((s) => (
                <label
                  key={s.id}
                  className="flex items-center justify-between text-xs border rounded-xl px-3 py-2"
                >
                  <span>
                    {s.servicePackageName} — còn {s.remainingCredits} lượt (HSD:{" "}
                    {s.expiresAt
                  ...
                  <input
                    ...
                    checked={!!s.autoRenew}
                    onChange={async (e) => {
                      await billingApi.setAutoRenew(s.id, e.target.checked);
                      const creditRes = await billingApi.credits();
                      if (creditRes.success && creditRes.data) {
                        setUserCredits(creditRes.data.remainingCredits ?? 0);
                        setLowCreditWarning(!!creditRes.data.lowCreditWarning);
      ...
      {activeView === "notifications" && <NotificationSettingsView />}
      {activeView === "family" && <FamilyProfilesView />}
      {activeView === "privacy" && <PrivacyRightsView />}
      {activeView === "guide" && <CaptureGuideView />}
