...
export const ConsultationChatModal: React.FC<ConsultationChatModalProps> = ({
  ...
  // 1. Fetch real chat history from DB on open and poll regularly for sub-second realtime sync
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        ...
        if (res.success && Array.isArray(res.data) && isSubscribed) {
          ...
          setMessages((prev) => {
            // Chỉ cập nhật nếu số lượng hoặc id tin nhắn cuối cùng thay đổi để tránh re-render giật lag
            if (
              prev.length === mapped.length &&
              prev.length > 0 &&
              prev[prev.length - 1].id === mapped[mapped.length - 1].id
            ) {
              return prev;
            }
            return mapped;
    ...
    // Thiết lập Polling liên tục 800ms để đảm bảo tin nhắn hiển thị tức thì 100% giữa 2 tab
    const pollInterval = setInterval(loadHistory, 800);
    ...
    if (currentUserId) {
      ...
      stompClient.subscribe(topic, (msg: any) => {
        if (msg && msg.messageText && isSubscribed) {
          ...
          const incoming: ChatMessage = {
            ...
            senderName:
              currentUserRole === "doctor" ? patientName : resolvedDoctorName,
    ...
    currentUserRole === "doctor"
      ? partnerUserId
        ? isVi
          ...
          : "Periodic fundus examination file"
        : isVi
          ? "Chưa liên kết tài khoản trực tuyến"
          : "No online account linked"
      : partnerUserId
        ? isVi
          ? "Bác sĩ phụ trách lâm sàng"
          : "Assigned clinical physician"
        : isVi
          ? "Chờ phân công Bác sĩ chuyên khoa phụ trách"
          : "Awaiting specialist physician assignment";
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    const optimisticMsg: ChatMessage = {
      ...
      senderName:
        currentUserRole === "doctor" ? resolvedDoctorName : patientName,
      text: textToSend,
      timestamp: new Date().toLocaleTimeString(isVi ? "vi-VN" : "en-US", {
        hour: "2-digit",
        minute: "2-digit",
      }),
    };
    ...
    try {
      ...
      if (res.success && res.data) {
        // Cập nhật id thật từ DB ngay khi gửi thành công
        setMessages((prev) =>
          prev.map((m) =>
            m.id === optimisticMsg.id ? { ...m, id: (res.data as any).id } : m,
          ),
        );
      } else {
  ...
  const quickRepliesDoctor = [
    isVi
      ...
      : "Diagnostic results have been clinically approved.",
    isVi
      ? "Bác nhớ đo huyết áp mỗi sáng và uống thuốc đều đặn."
      : "Please monitor blood pressure daily and take medications regularly.",
    isVi
      ? "Hình ảnh đáy mắt cho thấy vi tuần hoàn ổn định sau điều trị."
      : "Fundus imaging indicates stable microcirculation post-treatment.",
  ];

  const quickRepliesPatient = [
    isVi
      ? "Dạ bác sĩ cho tôi hỏi lịch tái khám cụ thể ạ."
      : "Doctor, could you advise on my follow-up schedule?",
    isVi
      ? "Tôi đã tải được báo cáo kết quả PDF rồi ạ."
      : "I have successfully downloaded the PDF report.",
    isVi
      ? "Cảm ơn bác sĩ đã tư vấn chi tiết."
      : "Thank you doctor for the detailed consultation.",
  ];

  const activeQuickReplies =
    currentUserRole === "doctor" ? quickRepliesDoctor : quickRepliesPatient;
    ...
    >
      <div className="space-y-4">
        ...
        <div className="p-2.5 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-800 flex items-center gap-2">
          ...
          <span>
            {isVi
              ? "Kênh trao đổi chuyên môn y khoa thời gian thực. Không sử dụng cho các trường hợp cấp cứu khẩn cấp."
              : "Real-time clinical consultation channel. Not intended for acute medical emergencies."}
          </span>
        </div>
        ...
        {!partnerUserId ? (
          <div className="py-8 px-6 text-center space-y-4 bg-slate-50/70 rounded-xl border border-clinical-border">
            ...
            <div className="space-y-2 max-w-md mx-auto">
              <h4 className="text-sm font-bold text-slate-900">
                {currentUserRole === "doctor"
                  ? isVi
                    ? "Bệnh nhân chưa liên kết tài khoản trực tuyến"
                    : "Patient has not linked online account"
                  : isVi
                    ? "Chưa có Bác sĩ chuyên khoa phụ trách"
                    : "No assigned specialist physician"}
              </h4>
              ...
              {currentUserRole !== "doctor" && (
                <div className="p-2.5 rounded-lg bg-blue-50/60 border border-blue-100 text-[11px] text-blue-900 leading-relaxed text-left space-y-1">
                  ...
                  <ul className="list-disc list-inside space-y-0.5 text-blue-800">
                    <li>
                      {isVi
                        ? "Sau khi Bác sĩ tiếp nhận hồ sơ, cửa sổ tư vấn 1-1 sẽ tự động kích hoạt."
                        : "Once a physician accepts the case, the 1-on-1 consultation window will activate automatically."}
                    </li>
                    <li>
                      {isVi
                        ? "Bạn có thể xem trước bản đồ nhiệt XAI và báo cáo sơ bộ tại cổng bệnh nhân."
                        : "You can review preliminary XAI heatmaps and reports in the patient portal in the meantime."}
                    </li>
                    <li>
                      {isVi
                        ? "Nếu có dấu hiệu giảm thị lực đột ngột hoặc đau nhức mắt, hãy đến ngay cơ sở y tế gần nhất."
                        : "If experiencing sudden vision loss or severe ocular pain, visit the nearest emergency room immediately."}
                    </li>
                  </ul>
                </div>
              )}
          ...
          <>
                ...
                messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div
                          className={`flex items-center justify-between gap-3 text-[10px] ${isMe ? "text-brand-100" : "text-slate-400"}`}
                        >
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[11px] font-semibold text-clinical-text-muted">
                {isVi ? "Gợi ý nhanh:" : "Quick replies:"}
              </span>
            ...
            <form onSubmit={handleSendMessage} className="flex gap-2 pt-2">
              <input
                ...
                placeholder={t(
                  "patient.chat.placeholder",
                  isVi
                    ? "Nhập nội dung tư vấn chuyên môn..."
                    : "Enter your question or symptoms for the doctor...",
                )}
