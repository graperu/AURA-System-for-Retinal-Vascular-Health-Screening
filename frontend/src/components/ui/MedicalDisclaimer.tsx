...
export const MedicalDisclaimer: React.FC<MedicalDisclaimerProps> = ({
  ...
  if (variant === 'banner') {
    return (
      <div
        role="note"
        aria-label="Cảnh báo an toàn y khoa CDS"
      ...
      >
        ...
        <div className="leading-relaxed">
          <strong className="font-bold block mb-0.5 text-amber-950">
            Tuyên bố Miễn trừ Y tế (Medical Safety Disclaimer):
          </strong>
          <span>{MANDATORY_MEDICAL_DISCLAIMER}</span>
        </div>
      </div>
    );
  }
  ...
  if (variant === 'subtle') {
    return (
      ...
      >
        ...
        <span>
          <strong className="font-semibold text-slate-700">Lưu ý y khoa bắt buộc:</strong> {MANDATORY_MEDICAL_DISCLAIMER}
        </span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <p>
        <strong className="font-semibold text-amber-950">Lưu ý y khoa bắt buộc:</strong> {MANDATORY_MEDICAL_DISCLAIMER}
      </p>
    </div>
  );
};
