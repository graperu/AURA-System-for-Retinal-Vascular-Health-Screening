...
export const PatientAssignmentBoard: React.FC = () => {
  ...
  const load = async () => {
    ...
    else setMessage(response.message || (isVi ? 'Không thể tải bảng phân công bệnh nhân.' : 'Failed to load patient assignment board.'));
  };
  ...
  const assign = async (doctorId: string, patientIds: string[]) => {
    ...
    } else setMessage(response.message || (isVi ? 'Phân công bệnh nhân không thành công.' : 'Patient assignment failed.'));
  };
  ...
  const unassign = async (doctorId: string, patientId: string) => {
    ...
    } else setMessage(response.message || (isVi ? 'Không thể hủy phân công.' : 'Failed to unassign patient.'));
  };
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <section onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const item = readDrag(event); if (item?.sourceDoctorId) void unassign(item.sourceDoctorId, item.patientId); }}
          ...
          <h3 className="mb-3 flex items-center justify-between text-sm font-bold text-slate-800">
            <span className="flex items-center gap-2"><UserRound className="h-4 w-4" /> {t('doctor.assignmentBoard.unassignedColumn', 'Chưa phân công')}</span>
            <span className="rounded-full bg-slate-200 px-2 py-0.5 text-xs">{unassigned.length}</span>
          </h3>
          <div className="space-y-2">{unassigned.map((patient) => <PatientCard key={patient.id} patient={patient} />)}</div>
          {unassigned.length === 0 && <p className="py-8 text-center text-xs text-slate-500">{t('doctor.assignmentBoard.allAssigned', 'Tất cả bệnh nhân đã có bác sĩ phụ trách.')}</p>}
        </section>
