...
export const DoctorConsultationView: React.FC<DoctorConsultationViewProps> = ({
  ...
  const currentDoctorName =
    doctorName ||
    user?.name ||
    (isVi ? "Bác sĩ chuyên khoa" : "Attending Specialist");
  const [selectedPatientId, setSelectedPatientId] = useState<string | null>(
    initialSelectedPatientId ||
      (assignedPatients.length > 0 ? assignedPatients[0].patientId : null),
  );
  ...
  const activePatient = useMemo(() => {
    return (
      assignedPatients.find((p) => p.patientId === selectedPatientId) ||
      assignedPatients[0] ||
      null
    );
  ...
  useEffect(() => {
    if (
      initialSelectedPatientId &&
      initialSelectedPatientId !== prevInitialPatientIdRef.current
    ) {
  ...
  useEffect(() => {
    ...
    loadConversation();
    const pollInterval = setInterval(loadConversation, 800);
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold text-[#134E4A]">
              {t(
                "doctor.consultation.title",
                "Kênh Tư Vấn & Trao Đổi Trực Tuyến Với Bệnh Nhân",
              )}
            </h1>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            {t(
              "doctor.consultation.subtitle",
          ...
          <div className="p-3.5 border-b border-slate-100 bg-slate-50/50 space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                ...
                {t(
                  "doctor.consultation.assignedPatients",
                  "Bệnh Nhân Phụ Trách",
                )}{" "}
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t(
                  "doctor.consultation.searchPlaceholder",
                  "Tìm theo tên, MRN, SĐT...",
                )}
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100 p-2 space-y-1">
            {filteredPatients.length === 0 ? (
              <div className="p-8 text-center text-xs text-slate-400">
                {t(
                  "doctor.consultation.noPatients",
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        <span
                          className={`text-xs font-bold truncate ${isSelected ? "text-[#134E4A]" : "text-slate-900"}`}
                        >
                          {p.fullName || (isVi ? "Bệnh nhân" : "Patient")}
                        </span>
                        <span className="text-[10px] text-slate-400 font-mono-data shrink-0">
                          {p.screeningCount
                            ? `${p.screeningCount} ${isVi ? "ca" : "scans"}`
                            : ""}
                        </span>
                      </div>
          ...
          {activePatient ? (
            <>
              ...
              <div className="p-3.5 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between gap-4">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {activePatient.latestRiskLevel && (
                        <RiskBadge
                          level={activePatient.latestRiskLevel}
                          size="sm"
                        />
                      )}
                    </div>
                    <div className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-3">
                      <span>
                        {isVi ? "HA:" : "BP:"}{" "}
                        <strong className="text-slate-700 font-mono-data">
                      ...
                      <span className="hidden sm:inline text-teal-700 font-semibold">
                        •{" "}
                        {t(
                          "doctor.consultation.attendingDoctor",
                          "Bác sĩ phụ trách",
                        )}
                        : {currentDoctorName}
                      </span>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      onSelectPatientForCDS(activePatient.patientId)
                    }
                    icon={<Stethoscope className="w-3.5 h-3.5" />}
                    title={t(
                      "doctor.consultation.openCdsTitle",
                      "Mở ảnh đáy mắt của bệnh nhân này trên bàn chẩn đoán CDS",
                    )}
                  >
                    <span className="hidden sm:inline">
                      {t("doctor.consultation.openCds", "Mở CDS")}
                    </span>
                  </Button>
                </div>
              </div>
              ...
              <div className="px-4 py-2 bg-amber-50 border-b border-amber-200/80 text-[11px] text-amber-900 flex items-center gap-2 shrink-0">
                ...
                <span>
                  <strong>
                    {t(
                      "doctor.consultation.safetyWarningTitle",
                      "Cảnh báo an toàn y khoa:",
                    )}
                  </strong>{" "}
                  {t(
                    "doctor.consultation.safetyWarningText",
                    "Kênh trao đổi chuyên môn y khoa thời gian thực (WebSocket). Không sử dụng cho các trường hợp cấp cứu khẩn cấp.",
                  )}
                </span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/40">
                {loadingHistory ? (
                  <div className="text-center py-12 text-xs text-slate-400">
                    {t(
                      "doctor.consultation.loadingHistory",
                  ...
                  <div className="text-center py-16 space-y-2">
                    ...
                    <h4 className="text-xs font-bold text-slate-700">
                      {t(
                        "doctor.consultation.noMessagesTitle",
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-3 text-slate-400">
              ...
              <p className="text-xs font-medium">
                {t(
                  "doctor.consultation.selectPatientPrompt",
                  "Vui lòng chọn một bệnh nhân ở cột bên trái để bắt đầu cuộc tư vấn.",
                )}
              </p>
            </div>
          )}
