...
class RestAuthenticationEntryPointTest {
    ...
    @ParameterizedTest
    @MethodSource("errors")
    void returnsSafeSpecificAuthenticationError(ErrorCode attribute, ErrorCode expected) throws Exception {
        ...
        entryPoint.commence(request, response, new InsufficientAuthenticationException("internal detail"));

        assertThat(response.getStatus()).isEqualTo(401);
        assertThat(response.getContentAsString()).contains(expected.name())
