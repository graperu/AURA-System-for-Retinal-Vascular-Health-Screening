...
class AuthIntegrationTest {
  ...
  void registerPersistsHashedPasswordAndUserRoleAndRejectsDuplicateEmail() throws Exception {
    ...
    String hash =
        jdbc.queryForObject("SELECT password_hash FROM users WHERE email = ?", String.class, EMAIL);
    Integer roleCount =
        jdbc.queryForObject(
            """
  ...
  void logoutNeedsNoAccessTokenRevokesRefreshAndClearsExactCookie() throws Exception {
    ...
    var result =
        mvc.perform(post("/api/v1/auth/logout").header("Origin", ORIGIN).cookie(cookie))
            .andExpect(status().isOk())
            .andReturn();

    String setCookie = result.getResponse().getHeader(HttpHeaders.SET_COOKIE);
    assertThat(setCookie)
        .contains(AuthController.COOKIE + "=")
  ...
  void protectedEndpointsReturn401And403Correctly() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/test/admin").header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
  ...
  private org.springframework.test.web.servlet.ResultActions register(String email)
    ...
    return mvc.perform(
        post("/api/v1/auth/register")
            ...
            .content(
                json.writeValueAsString(new Credentials(email, PASSWORD, "Integration User"))));
  }
