...
public class RefreshTokenService {
    ...
    public Issued issue(User user) {
        ...
        RefreshToken entity = repository.save(new RefreshToken(user, hash(raw),
    ...
    public Rotation rotate(String raw) {
        RefreshToken old = repository.findByTokenHashForUpdate(hash(raw))
                .orElseThrow(() -> invalid(ErrorCode.REFRESH_TOKEN_INVALID, "Refresh token không hợp lệ"));
    ...
    public void revoke(String raw) {
        repository.findByTokenHashForUpdate(hash(raw))
                .filter(token -> token.getRevokedAt() == null)
                .ifPresent(token -> token.revoke(null));
    }

    private AuthException invalid(ErrorCode code, String message) { return new AuthException(code, message); }
