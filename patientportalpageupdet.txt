...
export const PatientPortalPage: React.FC<PatientPortalPageProps> = ({
  ...
  // Auto-polling for consultation chat every 3s
  React.useEffect(() => {
  ...
  const handleStartAnalysis = async (request: FundusAnalysisRequest) => {
    ...
    try {
      ...
      // Save real screening to PostgreSQL DB via backend API
      try {
        const savedScreening = await screeningApi.create(result.imageUrl || request.imageUrl);
        console.log('Saved screening to DB:', savedScreening);
      } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    const csvRows = [
                      ['Mã Phân Tích', 'Ngày Khám', 'Vị Trí Soi', 'Điểm Nguy Cơ', 'Mức Độ', 'Nguy Cơ Tim Mạch', 'Bác Sĩ Phụ Trách', 'Trạng Thái'],
                      ...scanHistory.map((s) => [
                        s.id,
                        s.date,
                        s.eye,
                    ...
                    link.href = url;
                    link.download = `AURA_Scan_History_${patient.mrn}.csv`;
                    document.body.appendChild(link);
                    link.click();
                    document.body.removeChild(link);
                  }}
                  className="px-3.5 py-2 bg-white border border-slate-300 hover:bg-slate-100 text-slate-700 font-bold text-xs rounded-xl shadow-xs flex items-center gap-1.5 transition-all"
                >
                  <FileSpreadsheet className="w-4 h-4 text-emerald-600" /> Xuất Tất Cả (CSV)
                </button>
                <button
                  onClick={() => setIsReportModalOpen(true)}
                ...
                >
                  <Download className="w-4 h-4" /> Xuất Báo Cáo Ca Này (PDF/CSV)
                </button>
              </div>
            </div>
                    ...
                    <tr key={scan.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="p-3.5 text-right space-x-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-[#0891B2] hover:bg-[#0E7490] text-white font-bold rounded-lg text-xs shadow-xs transition-colors"
                        >
                          Xem Bản Đồ Nhiệt
                        </button>
                      </td>
      ...
      {activeView === 'consultation' && (
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[660px]">
            {/* Chat Header */}
            <div className="p-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <div className="w-10 h-10 rounded-xl bg-[#0891B2] flex items-center justify-center font-bold text-sm text-white shadow-sm">
                    BS
                  </div>
                  <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-400 border-2 border-slate-900 rounded-full" title="Đang trực tuyến"></span>
                </div>
                <div>
                  <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] text-cyan-200">Khoa Mắt & Tim Mạch Lâm Sàng • Trao đổi trực tuyến 2 chiều (FR-10)</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                ...
                >
                  <FileText className="w-3.5 h-3.5" /> Xem Báo Cáo Y Khoa
                </button>
            ...
            <div className="flex-1 p-5 overflow-y-auto space-y-4 bg-slate-50/80">
              <div className="text-center">
                <span className="rounded-full bg-teal-50 px-3 py-1 text-[10px] font-bold text-teal-800 border border-teal-200 font-mono-data">
                  Kênh tư vấn y khoa mã hóa bảo mật HIPAA • Tự động cập nhật realtime (3s)
                </span>
              </div>
            ...
            {/* Chat Input Bar */}
            <form onSubmit={handleSendChatMessage} className="p-3.5 bg-white border-t border-slate-200 flex items-center gap-2">
              <input
                type="text"
                value={newChatText}
                onChange={(e) => setNewChatText(e.target.value)}
                placeholder="Nhập tin nhắn trao đổi với Bác sĩ phụ trách..."
