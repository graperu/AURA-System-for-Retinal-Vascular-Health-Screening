...
| **FR-3** | **[Xem kết quả chẩn đoán]**: Hiển thị kết quả đánh giá nguy cơ tổng thể (Overall Risk Score) và chi tiết 4 phân nhóm: Tim mạch (CVD), Đột quỵ (Stroke), Tăng huyết áp (Hypertension), Bệnh võng mạc (Retinopathy). | **MOCK** | `frontend/src/pages/PatientPortalPage.tsx`<br>`frontend/src/components/RiskAssessmentPanel.tsx` | `POST /api/v1/screenings`<br>`GET /api/v1/screenings/{id}` | `backend/src/main/java/com/aura/screening/service/ScreeningService.java`<br>`ai-service/app/services/model_engine.py` | `screenings` | **Bằng chứng**: AI Service trả về các giá trị cố định (`avRatio=0.52`, `confidence=0.74`, `overallRiskScore=78`). Không có file trọng số `.pth` nào được nạp. | **P1** |
| **FR-4** | **[Trực quan hóa Grad-CAM]**: Hiển thị bản đồ nhiệt (Heatmap) đè lên ảnh gốc với thanh trượt chỉnh độ mờ (Opacity 0% - 100%) và công cụ zoom chi tiết. | **MOCK** | `frontend/src/components/InteractiveCDSViewer.tsx` | `/ai/api/v1/predict/upload` | `ai-service/app/services/model_engine.py` | Trả Base64 Image | **Bằng chứng**: Bản đồ nhiệt là mặt nạ Gaussian Blur (`cv2.GaussianBlur`) tâm cố định hòa trộn màu JET, **không phải Grad-CAM tính từ đạo hàm mạng nơ-ron PyTorch**. | **P1** |
| **FR-5** | **[Khuyến nghị sức khỏe tự động]**: Tự động sinh danh mục cảnh báo và lời khuyên y tế dựa trên mức độ rủi ro tính toán được. | **MOCK** | `frontend/src/pages/PatientPortalPage.tsx` | `GET /api/v1/screenings` | `backend/src/main/java/com/aura/screening/service/ScreeningService.java` | `screenings` (`findings`) | Các câu khuyến nghị được hardcode dạng chuỗi tĩnh trong file Python/Java theo 4 mức rủi ro, chưa có hệ thống Clinical Decision Rules chuyên sâu. | **P2** |
...
| **FR-8** | **[Quản lý hồ sơ y tế]**: Quản lý thông tin cá nhân, tiền sử bệnh án (đái tháo đường, huyết áp), tuổi và giới tính. | **UI ONLY** | `frontend/src/components/MedicalProfileModal.tsx` | `GET /api/v1/auth/me` | `backend/src/main/java/com/aura/auth/controller/AuthController.java` | `users` | Chỉ xem được thông tin tài khoản cơ bản. Form sửa tiền sử bệnh, tiểu đường, huyết áp chỉ cập nhật React state tạm thời, chưa có bảng `patient_medical_profiles` trong DB. | **P2** |
...
| **FR-11** | **[Mua gói cước & Credit]**: Chọn và thanh toán gói dịch vụ phân tích cá nhân qua cổng thanh toán mô phỏng/QR Code. | **MOCK** | `frontend/src/components/CreditPurchaseModal.tsx` | `POST /api/v1/me/packages/{id}/purchase` | `backend/src/main/java/com/aura/billing/service/BillingService.java` | `subscription`, `payment_transaction` | **Bằng chứng**: `PaymentGateway` đang dùng mock charge (`charge(email, amount)` luôn trả về success giả lập). Chưa tích hợp cổng thanh toán thật (VNPay/MoMo). QR code là mở rộng. | **P2** |
...
| FR | Yêu Cầu Chức Năng (Nguyên Văn Đề Bài) | Trạng thái | FE File / Component | API Endpoint | Backend File | Database Table | Hiện trạng thực tế & Bằng chứng kiểm thử | Ưu tiên |
| :--- | :--- | :---: | :--- | :--- | :--- | :--- | :--- | :---: |
| **FR-13** | **[Quản lý hồ sơ bệnh nhân]**: Truy cập và quản lý danh sách bệnh nhân được phân công tiếp nhận. | **PASS** | `frontend/src/pages/CDSDashboardPage.tsx` | `GET /api/v1/doctor/patients`<br>`GET /api/v1/doctor/patients/{id}`<br>`GET /api/v1/doctor/patients/{id}/screenings` | `backend/src/main/java/com/aura/doctor/controller/DoctorPatientController.java`<br>`backend/src/main/java/com/aura/auth/service/PatientAccessService.java` | `doctor_patient_assignments` (`V016`) | **PASS 100% End-to-End**: Bác sĩ chỉ xem và quản lý danh sách bệnh nhân được phân công tiếp nhận. Xóa bỏ hoàn toàn MOCK_PATIENTS. Đã chặn IDOR trên mọi API hồ sơ và ca sàng lọc. 15/15 ca kiểm thử bảo mật tự động đạt `PASS`. | **P1** |
| **FR-14** | **[Xem kết quả phân tích chuyên sâu]**: Xem các chỉ số hình thái học vi mạch: Tỷ lệ động/tĩnh mạch (AVR), độ xoắn vặn mạch máu (Tortuosity), hiện tượng bắt chéo động-tĩnh mạch (AV Nicking). | **PARTIAL** | `frontend/src/pages/CDSDashboardPage.tsx`<br>`frontend/src/components/InteractiveCDSViewer.tsx` | `GET /api/v1/screenings/{id}` | `backend/src/main/java/com/aura/screening/controller/ScreeningController.java` | `screenings` | Xem được ảnh và bảng chỉ số AVR, Tortuosity chi tiết trên giao diện, nhưng **chỉ số là dữ liệu mô phỏng từ AI Engine** (chưa có model weights thật). | **P2** |
...
| **NFR-15**| Chú thích lâm sàng rõ ràng trên Grad-CAM và chỉ số | **MOCK** | Bản đồ nhiệt là mặt nạ Gaussian mask mô phỏng, chưa phải Grad-CAM từ mô hình học sâu thật. |
...
| **NFR-22**| Kết quả dự đoán bắt buộc đính kèm Grad-CAM Explainable AI | **MOCK** | Chưa có Grad-CAM Explainable AI tính toán từ lớp Conv2D của mạng PyTorch. |
...
| :--- | :---: | :---: | :--- |
| **PASS** | **10** | 25.6% | FR-6 (Lịch sử khám), FR-12 (Lịch sử giao dịch), FR-16 (Ghi chú lâm sàng), FR-19 (Retraining feedback), FR-27 (Hạn mức Credit), FR-28 (Mua gói phòng khám), FR-31 (Quản lý User), FR-32 (Phân quyền RBAC), FR-34 (Quản lý Gói), FR-37 (Audit log). |
| **PARTIAL** | **12** | 30.8% | FR-1 (Auth thiếu Google), FR-2 (Upload thiếu DICOM/TIFF), FR-7 (Báo cáo thiếu PDF server), FR-9 (Thiếu Push/DB), FR-13 (Quản lý hồ sơ BN thiếu phân công), FR-14 (Xem kết quả - data mock), FR-15 (Xác nhận/hiệu chỉnh - ghi đè AI), FR-18 (Tìm kiếm nâng cao client-side), FR-22 (Đăng ký Clinic), FR-24 (Batch backend in-memory), FR-30 (Xuất CSV client), FR-36 (Phân tích lỗi AI). |
| **MOCK** | **4** | 10.3% | FR-3 (Kết quả AI cố định), FR-4 (Heatmap Gaussian mask), FR-5 (Khuyến nghị tĩnh), FR-11 (Payment Gateway giả lập). |
| **UI ONLY** | **10** | 25.6% | FR-8 (Hồ sơ y tế), FR-17 (Xem xu hướng), FR-21 (Thống kê hiệu suất Doctor), FR-23 (Quản lý Doctor & BN cơ sở), FR-25 (Giám sát rủi ro tổng hợp), FR-26 (Báo cáo chiến dịch), FR-29 (Cảnh báo nguy cơ cao), FR-33 (Ngưỡng AI), FR-35 (Dashboard tổng), FR-38 (Duyệt Clinic). |
...
| **MISSING** | **1** | 2.6% | FR-39 (Mẫu thông báo). |
| **TỔNG CỘNG** | **39 FR** | **100%** | |
...
| :--- | :---: | :---: | :--- |
| **PASS** | **7** | 30.4% | NFR-3 (Tải Dashboard $< 3$s), NFR-5 (AI Fallback Graceful FAILED), NFR-11 (Ẩn danh HMAC), NFR-13 (Responsive UI), NFR-14 (Thao tác $\le 3$ click), NFR-17 (Clean Architecture), NFR-21 (RESTful OpenAPI). |
...
| **MOCK** | **2** | 8.7% | NFR-15 (Chú thích Grad-CAM), NFR-22 (Grad-CAM Explainable AI). |
| **FAIL** | **1** | 4.3% | **P0 Lỗi Nghiêm Trọng**: NFR-12 (RBAC bị lỗ hổng IDOR). |
| **MISSING** | **1** | 4.3% | NFR-6 (Sao lưu PostgreSQL tự động hàng ngày). |
...
P0: Bảo Mật, Quyền Riêng Tư & Toàn Vẹn Dữ Liệu Y Tế (Khẩn Cấp)
 ...
 ├── [ ] P0-2: Sửa lỗ hổng IDOR tại GET /api/v1/screenings/{id}: Kiểm tra principal.id() == screening.patientId hoặc DOCTOR/ADMIN.
 ├── [ ] P0-3: Phân quyền ca khám Bác sĩ: Thêm bảng `doctor_patient_assignments` để bác sĩ chỉ xem ca được phân công (FR-13, FR-14).
...
P1: Hoàn Thiện Luồng Cốt Lõi (Core Flow Completion)
 ...
 └── Bổ sung bảng `patient_medical_profiles` lưu tiền sử bệnh án vào PostgreSQL (FR-8).
