...
| FR | Yêu Cầu Chức Năng (Nguyên Văn Đề Bài) | Trạng thái | FE File / Component | API Endpoint | Backend File | Database Table | Hiện trạng thực tế & Bằng chứng kiểm thử | Ưu tiên |
| :--- | :--- | :---: | :--- | :--- | :--- | :--- | :--- | :---: |
| **FR-22** | **[Đăng ký tài khoản tổ chức]**: Quy trình đăng ký và nộp hồ sơ xác thực pháp nhân phòng khám. | **DONE** | `frontend/src/pages/ClinicPortalPage.tsx` (`ClinicProfileSection`), `frontend/src/pages/AdminAuditLogsPage.tsx` (tab "Phê Duyệt Phòng Khám") | `POST/GET /api/v1/clinic/profile`, `GET /api/v1/admin/clinics`, `PATCH /api/v1/admin/clinics/{id}/verify` | `ClinicProfileController`, `ClinicProfileService` | `clinic_profiles` (migration `V018`) | Phòng khám nộp tên tổ chức, số giấy phép và file giấy phép hoạt động (ảnh/PDF); hồ sơ ở trạng thái `PENDING` → Admin xem, phê duyệt hoặc từ chối kèm lý do trên tab riêng; nộp lại sau khi bị từ chối sẽ đưa hồ sơ về `PENDING`. Có bộ test đơn vị `ClinicProfileServiceTest`. | — |
| **FR-23** | **[Quản lý Bác sĩ & Bệnh nhân]**: Gán quyền bác sĩ vào cơ sở, phân công bệnh nhân cho từng bác sĩ trực thuộc. | **DONE** | `frontend/src/pages/ClinicPortalPage.tsx` (`ClinicMembersSection`) | `GET/POST /api/v1/clinic/members`, `DELETE /api/v1/clinic/members/{id}`, `POST /api/v1/clinic/members/{doctorId}/patients/{patientId}` | `ClinicMemberController`, `ClinicMemberService` | `clinic_members` (migration `V018`) | Phòng khám mời bác sĩ đã có tài khoản `ROLE_DOCTOR` bằng email, gỡ (thu hồi) thành viên, và phân công bệnh nhân cho từng bác sĩ trực thuộc — có kiểm tra không cho phân công cho bác sĩ ngoài phòng khám hoặc đã bị thu hồi (đa nhiệm/multi-tenant isolation). Có bộ test đơn vị `ClinicMemberServiceTest` bao phủ các nhánh bảo mật này. | — |
...
| :--- | :---: | :---: | :--- |
| **PASS** | **14** | 35.9% | FR-6 (Lịch sử khám), FR-8 (Hồ sơ y tế và tệp xét nghiệm), FR-12 (Lịch sử giao dịch), FR-13 (Phân công và worklist), FR-16 (Ghi chú lâm sàng), FR-19 (Retraining feedback), FR-22 (Đăng ký & xác thực tổ chức phòng khám), FR-23 (Quản lý Doctor & BN cơ sở), FR-27 (Hạn mức Credit), FR-28 (Mua gói phòng khám), FR-31 (Quản lý User), FR-32 (Phân quyền RBAC), FR-34 (Quản lý Gói), FR-37 (Audit log). |
| **PARTIAL** | **11** | 28.2% | FR-1, FR-2, FR-5, FR-7, FR-9, FR-14, FR-15, FR-18, FR-24, FR-30, FR-36. |
| **MOCK** | **0** | 0% | Không còn luồng mock trong mã chạy production. |
| **UI ONLY** | **8** | 20.5% | FR-17 (Xem xu hướng), FR-21 (Thống kê hiệu suất Doctor), FR-25 (Giám sát rủi ro tổng hợp), FR-26 (Báo cáo chiến dịch), FR-29 (Cảnh báo nguy cơ cao), FR-33 (Ngưỡng AI), FR-35 (Dashboard tổng), FR-38 (Duyệt Clinic). |
...
> **Cập nhật FR-22 / FR-23**: Đã bổ sung tab "Phê Duyệt Phòng Khám" cho Admin (gọi trực tiếp `GET/PATCH /api/v1/admin/clinics`, trước đây các API này tồn tại ở backend nhưng không có giao diện nào gọi tới) và bộ test đơn vị cho `ClinicProfileService`/`ClinicMemberService` (trước đây module `clinic` không có test nào). Hai mục này chuyển từ `PARTIAL`/`UI ONLY` sang `PASS`.
